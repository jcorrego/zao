...
class Frontend
{
    ...
    )
    {
        ...
        global  $product ;
        if ( !isset( $product->id ) || defined( 'DOING_AJAX' ) && !$this->options['quick_view'] ) {
            return $located;
        }
        $run = get_post_meta( $product->id, '_checkbox_svipro_enabled', true );
        $has_images = $this->hasImages( $product->id );
        if ( empty($has_images) ) {
            return $located;
        }
    ...
    {
        global  $product ;
        echo  '<div data-sviproduct_id="' . $product->id . '" class="svi-vue-variationthumbs">' ;
    ...
    public function render_frontend( $divi_check = true )
    {
        ...
        if ( !$divi_check && strpos( $product->description, 'et_pb_wc_images' ) !== false ) {
...
<?php 
        wp_localize_script( 'svi-frontend', 'wcsvi_' . $product->id, json_encode( $this->parseData( $product->id ) ) );
        ?>
